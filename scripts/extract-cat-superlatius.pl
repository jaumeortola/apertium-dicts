...
while (my $line = <$fh>) {
    ...
    if ($line =~ /<pardef n="(.*__$apertium_gramcat)".*>/) {
        ...
        if ($line =~ /<e(.*?)>.*?<p>(.*?)(<\/l>|<l\/>).*<r>(.*?)(<.*>)<\/r><\/p><\/e>/) {
            ...
            if ($etiquetes =~ /"sup"/) { 
                $postag= "AQA".$genere.$nombre.$lt_post;
            }
...
while (my $line = <$fh>) {
    ...
    if ($line =~ /<e lm="(.*?)".*>.*<[il]>(.*?)<\/[il]>.*<par n="(.*__$apertium_gramcat)"\/><\/e>/) {
        ...
        if ($flexio_ap =~ /AQA/ ) {
            ...
            if ($flexio_ap =~ / AQ0FS0 (.*?) .*AQ0MS0 (.*?) .* AQAMS0 \b(.+)\b/) {
                #print "\n$line\n$flexio_ap\n";
                print "$2 $1 [sup. $3]=categories:A; font:apertium;\n";
            }
            if ($flexio_ap =~ / AQ0CS0 (.*?) .* AQAMS0 \b(.+)\b/) {
                #print "\n$line\n$flexio_ap\n";
                print "$1 [sup. $2]=categories:A; font:apertium;\n";
            }
        }
    }
}
